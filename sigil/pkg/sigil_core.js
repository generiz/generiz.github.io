 ...
 * @returns {string}
 */
export function sigil_demo_version() {
    let deferred1_0;
    let deferred1_1;
    try {
        const ret = wasm.sigil_demo_version();
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
...
/**
 * @param {string} input
 * @param {number} requested_loss
 * @returns {string}
 */
export function run_protocol_demo(input, requested_loss) {
    let deferred3_0;
    let deferred3_1;
    try {
        const ptr0 = passStringToWasm0(input, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.run_protocol_demo(ptr0, len0, requested_loss);
        var ptr2 = ret[0];
        var len2 = ret[1];
        if (ret[3]) {
            ptr2 = 0; len2 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('sigil_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
